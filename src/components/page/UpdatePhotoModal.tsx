...
const UpdatePhotoModal = forwardRef<UpdatePhotoModalRef, ModalProps>(({ className = '', style, onOpen, onClose, onUploadSuccess, setUser }, ref) => {
    ...
    const { upload, isUploading, error } = useUpload({
        onSuccess: async (url) => {
            const filedUpload = type === 'photoProfile' ? 'photoProfile' : 'photoCover';
            const user = await userService.updateUserProfile({ [filedUpload]: url });
            if (!user) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            ...
            >
                ...
                <div className="flex justify-between items-center bg-gray-200 p-4 h-16 rounded-t-lg">
                    <h2 className="text-lg font-semibold">{type === 'photoProfile' ? 'Upload Profile Photo' : 'Upload Photo Cover'}</h2>
