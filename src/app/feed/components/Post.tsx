...
const Comment: React.FC<CommentProps> = ({ user, content, time, likes, replies }) => (
  <div className="py-3">
    <div className="flex gap-3">
      <img src={user.image} alt={user.name} className="w-10 h-10 rounded-full object-cover flex-shrink-0" />
      <div className="flex-1">
        ...
        <div className="flex items-center gap-4 mt-1 text-sm text-gray-500">
          <button className="hover:text-gray-900">Like {likes && `(${likes})`}</button>
          <button className="hover:text-gray-900">Reply</button>
          {replies && (
            <button className="hover:text-gray-900">View {replies} replies</button>
          )}
          <span>{time}</span>
        </div>
      </div>
